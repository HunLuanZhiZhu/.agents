...
Use this policy when `first_round_contract_check=on`, when S2 generated sketches show duplicate arrows, missing dependency edges, ambiguous compound inputs, or repeated artifacts, or when a later stage explicitly relies on S2 topology. In default `contract_check_mode=final_only`, keep S2 checks lightweight and move exhaustive edge/replica verification to S6/S7 for the selected final figure.
...
- `sketch_edge_cardinality_contract`: for every required or optional connector, state `connector_id`, `source`, `target`, `required_or_optional`, `min_visible_instances`, `max_visible_instances`, `target_port`, `line_style`, and whether parallel duplicates are forbidden.
...
- `sketch_visible_edge_inventory_template`: a blank table S2 must fill after generation, listing every visible connector extracted from the returned image.

If any of these fields are missing, S2 must stop and request S1 repair before generating or continuing sketches.

## Edge Cardinality Rules

Every connector must have a maximum visual instance count. Default is exactly one visible instance:
...
If a required dependency edge is missing, ambiguous, hidden behind a label, or visually swallowed by a merge box, the sketch must be marked `FLAG_MAJOR` or `BLOCKED` unless a fresh regeneration repairs it.

## Compound Input Policy

Compound labels such as `Y_i + P_i + D_i` are high-risk because image models often treat them as new intermediate nodes. S1/S2 must decide the encoding before prompt generation.
...
3. `grouped_label_only`: place text like `Y_i + P_i + D_i` next to the module as a label. Do not draw separate arrows into the label and then another arrow from the label if that would create an unregistered intermediate node.
...
If a visible connector or artifact copy cannot be matched to the contract with low ambiguity, the sketch cannot be marked as clean. The audit should prefer a `FLAG_MAJOR` or `BLOCKED` status over accepting a false topology as `PASS`.
...
- "Use exactly one arrow for connector `<id>`."
...
- "Required dependency `<source -> target>` must be visible with arrowhead at `<target_port>`."
...
- a visible arrow has no connector ID in the contract;
