# Connector Provenance And Area Budget Policy v3.1.6 Hotfix

Use this policy when second-round strict contract checking is enabled, when repairing connector/area-budget faults, or during S6/S7 final contract generation and final audit. In default `contract_check_mode=final_only`, S2/S5 may use lighter prompt constraints and obvious-error screens, but S6/S7 must enforce the final selected figure contract.
...
Before sending any S2 or S5 prompt to Image Gen, the assistant must compile the candidate contract into an evidence-locked prompt package. For S2, first compile a `s2_pre_image_contract_sheet`, then compile `sketch_evidence_locked_prompt_package` from that sheet and the S1 sketch card. For S5 it is named `evidence_locked_prompt_package` and is compiled from the S4 formal candidate contract. This package is part of the text/image handoff and must be checked before generation.
...
If `prompt_ready_check` fails, do not call Image Gen. Repair S4 or the prompt first.
...
If any required field is missing or says fail, do not generate that S2 sketch. Repair S1 first.

The S2 prompt must explicitly tell Image Gen to prefer omitting an uncertain connector over inventing a plausible-looking connector. Any accidental connector that implies a false relation is a semantic failure, even if the sketch is otherwise visually useful.

S2 post-generation audit must flag and repair or regenerate sketches that:
...
For generated images, absence of the intended connector is usually less harmful than an invented connector. Prefer missing-safe repair over accepting a false relation.
...
When the target paper defines a distinctive system setting, role constraint, privacy/access boundary, topology, or deployment assumption, compact context cues may be necessary. They should not consume space needed to show the paper's core internal mechanisms.
...
## S7 Final Heavy Connector / Edge / Area Audit

S7 must run this section for every pending-submission figure. It is mandatory even when `contract_check_mode=final_only`; that mode disables S2/S5 heavy per-candidate audits only.

S7 must build a visible inventory from the pending-submission figure and compare it against S6 `final-figure-contract.md`:
...
The S7 heavy audit tables must include:
...
- disposition: pass, text patch, S7 internal repair, blocked, or max-repair.

S7 must mark `MAJOR-FIGURE-FIX` or `BLOCKER` when the pending-submission figure:
...
Caption and legend patches are allowed only for minor wording or explanation gaps after the visual connector/edge/area relation is already correct. They cannot convert a false connector, reversed edge, forbidden topology, misleading callout, occluded endpoint, or dominant side panel into a PASS. If one fresh S7 pending-figure regeneration can satisfy the S4/S6 contracts, return `S7-INTERNAL-REPAIR`; otherwise return the appropriate S7 blocked/max verdict.

## Core-First Repair Order

When an S5 candidate has both visual polish and semantic faults, repair in this order:
...
Do not mark an S5 candidate as clean if it still contains a known unsupported connector that could change the paper meaning. If it is kept only as a low-ranked candidate, the S5 report must label it `FLAG_MAJOR` or `BLOCKED` and preserve the connector risk for S6. Selecting such a candidate requires explicit risk discussion and confirmation.
