...
When a visual key, inset, callout, or detail panel is necessary, keep it subordinate to the figure body and make it carry information that is not already carried elsewhere. The governing check is the `new-information test`: if removing the element does not remove a distinct paper-relevant meaning, relation, orientation cue, or disambiguation cue, the element is redundant.
