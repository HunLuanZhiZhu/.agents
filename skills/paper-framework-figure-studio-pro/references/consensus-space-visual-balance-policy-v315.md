...
If any of these fields are missing, S2 or S5 should stop and request repair of the preceding text stage rather than generating images.
...
When the user asked for a complete paper framework/method overview diagram, scoped candidates are a minority exploration tool, not the default output. In the default 8-sketch S2 batch, at least 5 sketches must be complete-paper overview candidates and no more than 3 may be scoped probes. A scoped probe may contribute a visual sublanguage to S3, but S3 must expand or merge it back into a complete coverage plan before S4.
...
- using a small count badge instead of redrawing every client/classifier/sample;
...
Do not let background/context rows grow because they are visually easy to draw. A large context row is justified only when it adds a distinct paper relation that cannot be carried by a compact role strip, mini-map, legend, or caption. If a context row mainly says "there are clients" or "there is no server", compress it and spend the recovered area on core internal mechanism.
...
- heterogeneous local data roles, domains, clients, or sources;
...
- generated, retrieved, mixed, or repaired artifacts;
...
- aggregation, alignment, distillation, update, repair, or coordination action;
- every affected model family, module, agent, representation, or objective when the paper updates more than one.

If an S2/S5 candidate omits most anchors for a paper-primary space while still presenting itself as the whole framework, mark `<space>_underweighted=true` and repair the candidate prompt or previous text contract.
...
S3 should downgrade directions where repeated details in one path hide another paper-primary mechanism. S6/S7 should reject or route to repair any selected candidate whose title/caption claims multiple co-primary spaces/paths while the image visibly explains only one.
