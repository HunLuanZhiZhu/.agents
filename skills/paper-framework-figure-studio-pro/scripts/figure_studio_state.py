...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_record_generation.add_argument("--generator", required=True, choices=["imagegen", "create-image", "approved-image-api"])
    ...
    p_mark_s7 = sub.add_parser("mark-s7-run")
    add_common(p_mark_s7)
    p_mark_s7.add_argument("--run-id", required=True)
    p_mark_s7.add_argument("--mode", required=True, choices=sorted(S7_INTERNAL_RUN_MODES))
    p_mark_s7.add_argument("--status", required=True, choices=sorted(S7_INTERNAL_RUN_STATUS_VALUES))
    p_mark_s7.add_argument("--artifact-path", action="append", default=[])
    p_mark_s7.add_argument("--note", default="")
    p_mark_s7.set_defaults(func=cmd_mark_s7_run)

    p_scan_s7 = sub.add_parser("scan-s7")
    add_common(p_scan_s7)
    p_scan_s7.set_defaults(func=cmd_scan_s7)
