...
from .s7_internal import (
    cmd_mark_s7_run as s7_mark_run,
    cmd_scan_s7 as s7_scan,
)
...
def cmd_record_image_generation(args) -> int:
    run_dir = project_dir(args)
    state = load_state(run_dir)
    event = image_generation_event(args, run_dir)
...
def cmd_mark_s7_run(args) -> int:
    run_dir = project_dir(args)
    state = load_state(run_dir)
    row = s7_mark_run(args, run_dir, state)
    write_json(state_file(run_dir), state)
    print(json.dumps(row, indent=2, ensure_ascii=False))
    return 0


def cmd_scan_s7(args) -> int:
    run_dir = project_dir(args)
    state = load_state(run_dir)
    report = s7_scan(args, run_dir, state)
    write_json(state_file(run_dir), state)
    print(json.dumps(report, indent=2, ensure_ascii=False))
    return 0
...
def cmd_register_reference_images(args) -> int:
    ...
    profile["style_preference_scope_policy"] = (
        ...
        "figure-type and reader-effect suggestions. They must not automatically bias S2-S7."
    )
    profile["candidate_mix_policy"] = (
        "S2-SKETCH-EXPLORE defaults to 8 low-fidelity sketches; "
